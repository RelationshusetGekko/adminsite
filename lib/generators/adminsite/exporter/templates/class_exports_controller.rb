class Admin::<%= class_name.camelize %>ExportsController < AdminApplicationController
  before_filter :set_export_and_file_types
  before_filter :retrieve_file, :only => [:show, :destroy]

  def create
    Resque::enqueue(<%= class_name.camelize %>Export, params[:export_name_scope], params[:file_type])
  ...
  def retrieve_file
    params[:id].match /^(.*)_(.*)$/
    @export_file = <%= class_name.camelize %>Export.default_path_to_export.join("#{$1}.#{$2}")

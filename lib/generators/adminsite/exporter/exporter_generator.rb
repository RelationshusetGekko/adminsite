module Adminsite
  module Generators
    class ExporterGenerator < Rails::Generators::NamedBase
      ...
      def run_generation
        puts "Setting up Adminsite to export #{class_name.camelize} "

        template  "class_exports_controller.rb", "app/controllers/admin/#{file_name}_exports_controller.rb"
        template  "index.haml.erb", "app/views/admin/#{file_name}_exports/index.html.haml"
        template  "class_export.rb", "app/models/#{file_name}_export.rb"
        template  "exports.rb", "app/models/#{file_name}/exports.rb"
        inject_into_class "app/models/#{file_name}.rb", class_name do
          "  include #{class_name.camelize}::Exports\n"
        end
        ...
        inject_into_file "app/views/admin/shared/_menu.haml", :before => /^.*destroy_admin_session_path/ do
          "    = menu_item '#{class_name.camelize} exports', admin_#{file_name}_exports_path, '#{file_name}_exports'\n"
        end
        # Gemfile

namespace :app do
  namespace :<%= class_name.underscore %> do
    ...
    desc 'Load <%= class_name.camelize %> data'
    task :import => :environment do

      puts "Before import, we have #{<%= class_name.camelize %>.count} <%= class_name.camelize %>s in DB"

      counter = 0;
      path = Rails.root.join('db', 'seeds', '<%= class_name.underscore %>s')
      <%= class_name.camelize %>.transaction do
        Dir[path.to_s + "/**"].each do |filename|
          basename = File.basename(filename)
          puts "Import #{basename}"
          row_index = 0
          import_attributes = <%= class_name.camelize %>.new.attributes.keys
          FasterCSV.foreach(filename,
            ...
            row_index += 1
            begin
              <%= class_name.underscore %> = <%= class_name.camelize %>.create!(row.to_hash)
